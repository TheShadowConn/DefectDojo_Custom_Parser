...
class DevoteamScanResultV3Parser:
    ...
    def get_findings(self, filename, test):
        ...
        for item in items:
            ...
            host_identifier = asset
            
            if host_identifier == "N/A":
                host_identifier = ip
            
            if host_identifier == "N/A":
                host_identifier = url
            
            if host_identifier and host_identifier != "N/A":
                title = (
                      f"{title} "
                      f"[{host_identifier}] "
                      f"[{vulnerability_id}]"
...
""".strip()
    ...
    def generate_unique_id(self, vulnerability_id, asset, title, severity, cvss, description=""):
        raw_value = f"{vulnerability_id}|{asset}|{title}|{severity}|{cvss}|{description}"
