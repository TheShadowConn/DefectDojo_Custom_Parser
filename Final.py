...
class IngestV3Parser:
    ...
    def get_findings(self, filename, test):
        ...
        for item in items:
            ...
            self.attach_**cations_and_endpoints(
          **    finding=finding,
            **  url=url,
                ip=ip,**               asset=asset,
     **     )

            findings.appe**(finding)

        return finding**
    def attach_locations_and_end**ints(self, finding, url, ip, asse**:
        targets = self.build_ta**ets(url=url, ip=ip, asset=asset)
**       locations = []
        end**ints = []
